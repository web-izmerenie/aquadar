...
$arFieldsCodeName = $arFiles = array();


// boundary
$mime_boundary = "------------{". md5(time()) ."}";

if($_SERVER['REQUEST_METHOD'] == 'POST' && strlen($_REQUEST['submit']) > 0 && $arParams['UNIQUE_FORM_ID'] == $_REQUEST['UNIQUE_FORM_ID'])
{
	...
	if(check_bitrix_sessid())
	{
		...
		if(!empty($arParams['CUSTOM_FIELDS']))
		{
			...
			foreach($arParams['CUSTOM_FIELDS'] as $k => $fv)
			{
				...
				{
                    if($arParams['REPLACE_FIELD_FROM'])
                        $arParams['EMAIL_FROM'] = $field_val[$k];

                    $_REQUEST['author_email'] = $field_val[$k];
		...
		if(empty($arResult))
		{
			$arFields = Array(
				...
				'AUTHOR_WORK_CITY'       => htmlspecialcharsEx(strip_tags($_REQUEST['author_work_city'])),
				...
				'PAGE_URL'               => $arParams['HTTP_PROTOCOL'] . $_SERVER['HTTP_HOST'] . $APPLICATION->GetCurUri(),
			...
			if(empty($arResult['ERROR_MESSAGE']))
			{
				...
				if(!empty($arParams['ADMIN_EVENT_MESSAGE_ID']))
				{
					...
					{
						if(IntVal($v) > 0)
							if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arCustomFields,$arFields), 'N', IntVal($v), false, $mime_boundary, $arFieldsCodeName, $arParams))
								$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');
					}
				}
				else if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arCustomFields,$arFields), 'N', false, $mime_boundary, $arFieldsCodeName, $arParams))
					$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');
				...
				if(!empty($arParams['USER_EVENT_MESSAGE_ID']) && !empty($arFields['AUTHOR_EMAIL']))
				{
					...
					{
						if(IntVal($v) > 0)
							if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arCustomFields,$arFields), 'N', IntVal($v), true, $mime_boundary, $arFieldsCodeName, $arParams))
								$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');
					}
				}
...
{
	$TICKET_ID = intval($_SESSION['API_MAIN_FEEDBACK']['TICKET_ID']) ? $_SESSION['API_MAIN_FEEDBACK']['TICKET_ID'] : '';
	$arResult['OK_MESSAGE']                   = str_replace('#TICKET_ID#',$TICKET_ID,$arParams['OK_TEXT']);
	$_SESSION['API_MAIN_FEEDBACK']['SUCCESS'] = false;
	$_SESSION['API_MAIN_FEEDBACK']['TICKET_ID'] = false;
...
else
{
	...
	if($arParams['CUSTOM_FIELDS'])
	{
		foreach($arParams['CUSTOM_FIELDS'] as $k1 => $v1)
		{
			...
			if(in_array('text',$arExplodeFields) || in_array('hidden',$arExplodeFields) || in_array('coupon',$arExplodeFields) || in_array('date',$arExplodeFields) || in_array('datetime',$arExplodeFields) || in_array('textarea',$arExplodeFields))
            {
                if($arExplodeFields)
                {
	...
	//v1.3.4
	if($USER->IsAuthorized() && !empty($arParams['DISPLAY_FIELDS']))
	{
		...
		if($arUser = $rsUser->GetNext(true,false))
		{
            ...
            $arResult['AUTHOR_FIO'] = trim($arUser['LAST_NAME'] . ' ' . $arUser['NAME'] . ' ' . $arUser['SECOND_NAME']);
			...
			$arResult['AUTHOR_WORK_CITY']       = $arUser['WORK_CITY'];
		}
	}
}
